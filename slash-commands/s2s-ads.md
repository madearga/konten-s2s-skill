---
description: "Ad / Commercial Motion Prompt (Dexhunter 12-pattern library)"
argument-hint: "[args]"
---

Execute the **s2s ads** capability from the `storyboard-to-seedance-suite` skill.
...
Read `$SKILL/commands/ads.md` and follow its instructions exactly. Resolve every relative reference it points to (e.g. `references/*.md`, `[ref:...]`) against `$SKILL/`. Consult `$SKILL/SKILL.md` for the capability router / load map if needed.
