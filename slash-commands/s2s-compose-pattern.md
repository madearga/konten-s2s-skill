---
description: "Pattern-based motion prompt (extend/edit/fuse/beat-sync/dialogue/one-take)"
argument-hint: "[args]"
---

Execute the **s2s compose-pattern** capability from the `storyboard-to-seedance-suite` skill.
...
Read `$SKILL/commands/compose-pattern.md` and follow its instructions exactly. Resolve every relative reference it points to (e.g. `references/*.md`, `[ref:...]`) against `$SKILL/`. Consult `$SKILL/SKILL.md` for the capability router / load map if needed.
